...
async fn run_scheduled_job_internal(
    ...
    if let Some(recipe_extensions) = recipe.extensions {
        // Add all extensions concurrently for faster startup
        let results = agent.add_extensions(recipe_extensions.clone()).await;
        
        // Check for any failures
        for (ext_name, result) in results {
            result.map_err(|e| JobExecutionError {
                job_id: job.id.clone(),
                error: format!("Failed to add extension '{}': {}", ext_name, e),
            })?;
        }
    }
